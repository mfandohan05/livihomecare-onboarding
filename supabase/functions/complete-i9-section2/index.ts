...
Deno.serve(async (req) => {
  ...
  try {
    ...
    setText("Last Name First Name and Title of Employer or Authorized Representative", adminName);
    setText("Signature of Employer or AR", adminName);
    setText("S2 Todays Date mmddyyyy", today);
    setText("Employers Business or Org Name", "MDC Global Care Solutions dba Livi Home Care");
    setText("Employers Business or Org Address", "179 Gasoline Alley Dr. Suite 203, Mooresville NC 28117");
    ...
    const { error: uploadError } = await supabase.storage
      .from("generated-pdfs")
      .upload(`${caregiverId}/i9_completed.pdf`, filledPdfBytes, {
        contentType: "application/pdf",
        upsert: true,
      });

    if (uploadError) throw new Error(`Could not save I-9: ${uploadError.message}`);
