...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // load existing caregiver-signed PDF
    ...
    if (documentType === 'drug_test_policy_signed') {
      ...
      page.drawText(adminName, {
        x: 190, y: h - 683,
        size: 11, font: italic, color: rgb(0, 0, 0)
      })
      // LHC Date
      page.drawText(todayStr(), {
        x: 410, y: h - 683,
        size: 11, font: regular, color: rgb(0, 0, 0)
      })
    }
    ...
    await supabase.storage
      .from('generated-pdfs')
      .upload(filePath, saved, { contentType: 'application/pdf', upsert: true })
